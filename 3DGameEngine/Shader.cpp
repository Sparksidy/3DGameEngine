...
namespace GameEngine { namespace Graphics {
	...
	Shader::Shader(const char* vertex, const char* fragment):m_VertPath(vertex), m_FragPath(fragment)
	{
		m_ShaderID = load();
	...
	GLuint Shader::load()
	{
		...
		std::string vertexSourceString = FileUtils::read_file(m_VertPath);
		std::string fragmentSourceString = FileUtils::read_file(m_FragPath);
